...
let Manager = (() => {
    let _classDecorators = [withEmployementDatePrototype, withEmployementDate, printData];
...
function withEmployementDatePrototype(value, context) {
    //   console.log("[withEmployementDatePrototype] was invoked!");
    value.prototype.employmentDate = new Date();
}
function withEmployementDate(baseClass, context) {
    console.log("[withEmployementDate] was invoked!");
    return class extends baseClass {
        constructor(...args) {
            super(...args);
            this.employmentDate = new Date().toISOString();
            console.log(`Added employement date property to ${baseClass.name} class! These are the args: ${args}`);
        }
    };
}
// Decorator 1 -> prints meta data
function printData(value, context) {
